            ...
            {weekDates ? weekDates.map((date, i) => (
              <div key={i} className="day-col">
                <div className="day-header">
                  ...
                  <div className={`day-num ${currentTodayStr === date.toISOString().split('T')[0] ? 'today' : ''}`}>{date.getDate()}</div>
                </div>
...
export default function Dashboard() {
  ...
  const weekDates = useMemo(() => getWeekDates(now ? new Date(dailyTimestamp) : null), [dailyTimestamp]);
