...
export default function Dashboard() {
  ...
  const weeklyScore = useMemo(() => weeklyStats.completed + bestStreak * 25 + (habitsWithProgress.filter((h: any) => h.weekProgress > 50).length * 50), [weeklyStats.completed, bestStreak, habitsWithProgress]);

  const dailyTimestamp = now ? new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime() : 0;
  const weekDates = useMemo(() => getWeekDates(now), [dailyTimestamp]);
