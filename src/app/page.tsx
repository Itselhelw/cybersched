...
const AnalyticsSection = memo(function AnalyticsSection({ tasks, habits, settings, smokeStats }: { tasks: Task[]; habits: Habit[]; settings: Settings; smokeStats: SmokeStats }) {
  ...
  // Optimization: Memoized to prevent O(N) array processing on every render
...
export default function Dashboard() {
  ...
  const {
    ...
    todayTasks, // Pre-sorted and memoized
  ...
  // Gamification calculations - memoized to prevent re-renders on every clock tick
  // Optimization: Reduces expensive O(N) recalculations from 60/min to only on data change.
