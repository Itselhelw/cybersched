...
// ── TASKS SECTION ─────────────────────────────────────────────────
// Memoize component to skip re-renders when parent's per-second clock updates
...
// ── PLANNER SECTION ───────────────────────────────────────────────
// Memoize component and use stable props (weekDates, currentTodayStr)
// to prevent per-second re-renders from the parent clock.
...
export default function Dashboard() {
  ...
  // Gamification calculations - memoized to prevent re-renders on per-second clock ticks
  ...
  // Derived stable date markers for component memoization
  const dailyDate = useMemo(() => {
    if (!now) return null;
    return new Date(now.getFullYear(), now.getMonth(), now.getDate());
  }, [now ? todayStr(now) : null]);
