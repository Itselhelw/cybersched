            ...
            {weekDates ? weekDates.map((date, i) => (
              <div key={i} className="day-col">
                <div className="day-header">
                  ...
                  <div className={`day-num ${dailyDate && date.getTime() === dailyDate ? 'today' : ''}`}>{date.getDate()}</div>
                </div>
...
export default function Dashboard() {
  ...
  // Stable daily timestamp for components that only need daily updates
  const dailyDate = useMemo(() => now ? new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime() : 0, [now ? currentTodayStr : null]);
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      <main style={{ marginLeft: 72, flex: 1, padding: 32, maxWidth: 'calc(100vw - 72px)', minWidth: 0 }}>
        ...
        {activeNav === 'planner' && <PlannerSection addTask={addTask} notify={notify} aiSchedule={aiSchedule} setAiSchedule={setAiSchedule} dailyDate={dailyDate} weekDates={weekDates} />}
