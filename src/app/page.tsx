...
const PlannerSection = memo(function PlannerSection({ addTask, notify, aiSchedule, setAiSchedule, dailyDate }: {
  ...
  const now = useMemo(() => new Date(dailyDate), [dailyDate]);
            ...
            {weekDates ? weekDates.map((date, i) => (
              <div key={i} className="day-col">
                <div className="day-header">
                  ...
                  <div className={`day-num ${date.getDay() === now.getDay() && date.getDate() === now.getDate() ? 'today' : ''}`}>{date.getDate()}</div>
                </div>
...
export default function Dashboard() {
  ...
  const dailyDate = useMemo(() => now ? new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime() : 0, [now?.getDate(), now?.getMonth(), now?.getFullYear()]);
  const weekDates = useMemo(() => getWeekDates(now ? new Date(dailyDate) : null), [dailyDate]);
