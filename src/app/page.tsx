...
export default function Dashboard() {
  ...
  useEffect(() => {
    const checkMidnight = () => {
      ...
      if (lastActiveDate && lastActiveDate !== today) {
        ...
        app.setTasksRaw(prev => prev.filter((t: Task) => !t.done));

        // 2 — Reset habit todayDone flags for new day
        app.setHabitsRaw(prev => prev.map((h: Habit) => ({ ...h, todayDone: false })));
  ...
  const {
    ...
    setTasksRaw, setHabitsRaw, setQuitDate
  } = app;
  ...
  // Gamification calculations - memoized to prevent recalculation on every clock tick
  ...
  // Use a stable daily date for week calculation to prevent prop instability
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      <main style={{ marginLeft: 72, flex: 1, padding: 32, maxWidth: 'calc(100vw - 72px)', minWidth: 0 }}>
        ...
        {activeNav === 'tasks' && <TasksSection tasks={tasks} setTasks={setTasksRaw} currentTodayStr={currentTodayStr} />}
        {activeNav === 'habits' && <HabitsSection habits={habitsWithProgress} setHabits={setHabitsRaw} toggleHabit={toggleHabit} />}
