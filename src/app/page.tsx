...
const AnalyticsSection = memo(function AnalyticsSection({ tasks, habits, settings, smokeStats }: { tasks: Task[]; habits: Habit[]; settings: Settings; smokeStats: SmokeStats }) {
  ...
  // Memoize analytics data to prevent O(N) recalculations on every dashboard update
            ...
            {weekDates ? weekDates.map((date, i) => (
              <div key={i} className="day-col">
                <div className="day-header">
                  ...
                  <div className={`day-num ${todayStr(date) === currentTodayStr ? 'today' : ''}`}>{date.getDate()}</div>
                </div>
...
export default function Dashboard() {
  ...
  // Memoize date strings using stable today marker to prevent re-renders on clock tick
  const { displayDay, displayMonth, displayDate, displayYear } = useMemo(() => ({
    displayDay: now ? DAYS[now.getDay()] : '',
    displayMonth: now ? MONTHS[now.getMonth()] : '',
    displayDate: now ? now.getDate() : '',
    displayYear: now ? now.getFullYear() : '',
  }), [currentTodayStr]);

  // Memoize gamification calculations to prevent O(N) calculations on every clock tick
  const { dailyScore, bestStreak, weeklyScore } = useMemo(() => {
    ...
    return { dailyScore, bestStreak, weeklyScore };
  }, [completedToday, totalToday, habitsWithProgress, currentTodayStr]);

  // Memoize weekDates and sorted tasks for today to prevent O(N) calculations and sorting on every clock tick
