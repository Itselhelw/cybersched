...
export const WeeklyProgressChart = React.memo(function WeeklyProgressChart({ data }: { data: Array<{ day: string; completed: number; total: number }> }) {
  return (
...
export const CategoryBreakdownChart = React.memo(function CategoryBreakdownChart({ data }: { data: Array<{ name: string; completed: number; total: number; color: string }> }) {
  return (
    <div className="card" style={{ padding: 20, marginBottom: 20 }}>
      <div className="card-header" style={{ marginBottom: 20 }}>
...
export const StreakRanking = React.memo(function StreakRanking({ data }: { data: Array<{ name: string; streak: number; color: string }> }) {
  return (
    <div className="card" style={{ padding: 20, marginBottom: 20 }}>
      <div className="card-header" style={{ marginBottom: 20 }}>
...
export const CompletionDonut = React.memo(function CompletionDonut({ stats }: { stats: { total: number; completed: number; pending: number; percentage: number } }) {
  const data = [
