...
export function useAppState(now: Date | null) {
    ...
    // ── HABIT SYNC ────────────────────────────────────────────────
    // Memoize the daily timestamp and today string to stabilize callbacks and effects
    ...
    const syncTasksToHabits = useCallback(() => {
        ...
        // Update all habits based on current tasks in a single pass
        setHabitsRaw(prev => prev.map(h => {
            ...
            // Streak logic: increment if newly done today, keep if already done
